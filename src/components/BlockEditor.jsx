...
function BloqueCard({
  ...
  const hijos = secuenciaDesde(
    programa.bloques,
    (bloque.hijosId || [])[0]
  );

  const els = def.tieneElse
  ...
  return (
    <div className={`rounded-xl transition ${activo ? "ring-2 ring-amber-400 ring-offset-1" : ""}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto flex items-center gap-1">
            <button
              onClick={(e) => {
                e.stopPropagation();
                setPrograma(moverBloque(programa, bloque.id, -1));
              }}
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                setPrograma(eliminarBloque(programa, bloque.id));

                if (seleccionado === bloque.id) {
      ...
      {esCont && (
        <div
          className="ml-4 mt-1.5 border-l-2 border-dashed pl-3"
          style={{ borderLeftColor: cat.hex + "80" }}
        >
          ...
          {hijos.length === 0 ? (
            <button
              onClick={() =>
                setPrograma(
                  agregarBloque(programa, "salida", "hijos", bloque.id)
                )
              }
            ...
            <div className="mt-2">
              ...
              {els.length === 0 ? (
                <button
                  onClick={() =>
                    setPrograma(
                      agregarBloque(programa, "salida", "else", bloque.id)
                    )
                  }
                  className="flex w-full items-center justify-center gap-1 rounded-lg border border-dashed border-slate-200 py-2 text-xs text-slate-400 hover:border-indigo-300 hover:text-indigo-500"
                >
...
export default function BlockEditor({
  ...
  const [modoControl, setModoControl] = useState("teclado");
  const [zona, setZona] = useState("lienzo");
  ...
  // Control real de la ejecución.
  const ejecucionAbortRef = useRef(null);
  ...
  const catsConBloques = Object.entries(CATEGORIAS).filter(([catKey]) =>
    Object.values(BLOQUES).some(
      (b) => b.categoria === catKey && permitidos.includes(b.tipo)
    )
  );
  ...
  const ejecutar = useCallback(async () => {
    if (!validacion.ok || corriendoRef.current) return;

    const controller = new AbortController();
    ejecucionAbortRef.current = controller;
      ...
      await ejecutarPrograma(programa, {
        ...
        onError: (msg) => {
          if (!controller.signal.aborted) {
            appendConsola("✖ " + msg, "error");
          }
        },

        onDone: () => {
          if (!controller.signal.aborted) {
            appendConsola("✔ Ejecución finalizada.", "ok");
          }
        },
      });
    } finally {
      ejecucionAbortRef.current = null;
      setInputDialog(null);
      setBloqueActivo(null);
      setCorriendo(false);
      corriendoRef.current = false;
    }
  }, [programa, validacion.ok, appendConsola]);

  const detener = useCallback(() => {
    const controller = ejecucionAbortRef.current;
    ...
    appendConsola("⏹ Detenido por el usuario.", "info");
  }, [inputDialog, appendConsola]);

  const ordenIds = (() => {
    const ini = programa.bloques.find((b) => b.tipo === "inicio");

    const seq = secuenciaDesde(
      programa.bloques,
      ini ? ini.siguienteId : null
    );

    return [ini?.id, ...seq.map((b) => b.id)].filter(Boolean);
  })();

  const ordenIdsRef = useRef(ordenIds);
  ordenIdsRef.current = ordenIds;
  ...
  const dispararGesto = useCallback(
    (id) => {
      if (id === "pulgar") {
        ejecutar();
      }
        ...
        if (objetivo) {
          ...
          appendConsola(
            "Bloque eliminado (gesto).",
            "info"
          );
        } else {
          appendConsola(
            "Nada que eliminar: mueve el cursor a un bloque primero.",
            "info"
          );
        }
      }
        ...
        if (f) {
          ...
          appendConsola(
            "Bloque seleccionado (gesto).",
            "info"
          );
        } else {
          appendConsola(
            "Mueve el cursor a un bloque y luego pellizca.",
            "info"
          );
        }
      }
    },
    [ejecutar, detener, appendConsola]
  );

  useEffect(() => {
    if (!gestoActivo) {
      setGestoProgreso(0);
      gestoConsumidoRef.current = false;
      return;
    }
    ...
    return () => clearInterval(t);
  }, [gestoActivo, moverCursor, dispararGesto]);
  ...
  const inicio = programa.bloques.find(
    (b) => b.tipo === "inicio"
  );

  const mainSeq = secuenciaDesde(
    programa.bloques,
    inicio ? inicio.siguienteId : null
  );
  ...
  return (
    <div className="flex h-[calc(100vh-9rem)] flex-col gap-3 lg:flex-row">
      ...
      {/* PALETA */}
      <div
        className={`flex w-full shrink-0 flex-col rounded-2xl border border-slate-200 bg-white shadow-sm lg:w-64 ${
          zona === "paleta"
      ...
      >
        <div className="border-b border-slate-100 px-4 py-3">
          ...
          <p className="text-[11px] text-slate-400">
            Clic en un bloque para agregarlo
          </p>
        </div>
        ...
        <div className="flex-1 space-y-2 overflow-y-auto p-3">
          {catsConBloques.map(
            ([catKey, cat]) => {
              const bloquesCat =
                Object.values(BLOQUES).filter(
                  (b) =>
                    b.categoria === catKey &&
                    permitidos.includes(b.tipo)
                );

              if (!bloquesCat.length) {
                return null;
              }

              const abierta =
                categoriaAbierta === catKey;
                          ...
                          <button
                            ...
                            title={
                              ...
                              programa.bloques.some(
                                ...
                                ? "Ya existe un bloque de este tipo"
                                : b.descripcion
                            }
                          >
                            <Icon
      ...
      {/* LIENZO */}
      <div
        className={`flex min-w-0 flex-1 flex-col rounded-2xl border border-slate-200 bg-slate-50 shadow-sm ${
          zona === "lienzo"
            ? "ring-2 ring-teal-400"
            : ""
        }`}
        ...
        <div className="flex items-center justify-between border-b border-slate-100 bg-white px-4 py-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            {corriendo && (
              <button
                onClick={detener}
                className="flex items-center gap-1.5 rounded-lg bg-rose-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-rose-600"
              >
                <Square size={13} />
                Detener
              </button>
        ...
        >
          {programa.bloques.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center text-slate-400">
              ...
              <p className="text-xs">
                Toca un bloque de la paleta para
                añadirlo al lienzo
              </p>
            </div>
          ) : (
            <div className="space-y-1.5">
              {inicio ? (
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-1 rounded-xl border-2 border-dashed border-emerald-300 py-3 text-sm font-medium text-emerald-600 hover:bg-emerald-50"
                >
                  <Plus size={16} />
                  Agregar bloque Inicio
                </button>
              )}
              ...
              {inicio && (
                <button
                  ...
                  className="flex w-full items-center justify-center gap-1 rounded-lg border border-dashed border-slate-200 py-1.5 text-xs text-slate-400 hover:border-indigo-300 hover:text-indigo-500"
                >
                  <Plus size={12} />
                  Agregar bloque aquí
                </button>
              ...
              {programa.bloques.find(
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-1 rounded-xl border-2 border-dashed border-emerald-300 py-2 text-sm font-medium text-emerald-600 hover:bg-emerald-50"
                >
                  <Plus size={15} />
                  Agregar bloque Fin
                </button>
      ...
      <div className="flex w-full shrink-0 flex-col gap-3 lg:w-80">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          ...
          <p className="mt-2 text-[10px] text-slate-400">
            Mano abierta / índice: mueven el cursor
            (mantén el gesto para seguir moviéndote).
            Pinza: selecciona. Dos dedos: elimina.
            Pulgar: ejecuta. Puño: cancela.
            El vídeo no se envía a ningún servidor.
          </p>
        </div>
        ...
        {enunciado && (
          <div className="rounded-2xl border border-indigo-100 bg-indigo-50/50 p-3">
            <p className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-indigo-500">
              Enunciado
            </p>
        ...
        {/* CONSOLA */}
        <div
          className={`flex min-h-[160px] flex-1 flex-col rounded-2xl border border-slate-200 bg-slate-900 shadow-sm ${
            zona === "consola"
              ? "ring-2 ring-teal-400"
              : ""
          }`}
